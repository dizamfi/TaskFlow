...
export function useNotifications() {
  ...
  // Generar notificaciones automáticas basadas en tareas
  const generateTaskNotifications = useCallback((tasks: Task[]) => {
    const now = new Date();
    const newNotifications: Notification[] = [];
  ...
  useEffect(() => {
    if (user) {
      ...
      // Simular carga de notificaciones desde el servidor
      setTimeout(() => {
        const mockNotifications: Notification[] = [
          {
            ...
            message: 'Comienza creando tu primera tarea para organizarte mejor.',
            timestamp: new Date().toISOString(),
            read: false,
          },
        ];
